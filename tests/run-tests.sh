...
fire() {
  ...
  env WAKEUP_LOG="$LOG" WAKEUP_LOCK="$LOCK" WAKEUP_STATUS="$STATUS" WAKEUP_DRY_RUN=1 \
      WAKEUP_CONFIG="$ROOT/config.json" "$@" \
...
echo
echo "claude-code-wakeup — logic tests (macOS)"
echo
...
# 10 -------------------------------------------------------------------------
setup t10
video="$(env WAKEUP_LOG="$LOG" WAKEUP_CONFIG="$ROOT/config.json" bash -c '. '"$ROOT"'/lib/common.sh; resolve_video')"
...
# 11 — enabled:false skips ---------------------------------------------------
setup t11
fire "$FIX/permission_prompt.json" WAKEUP_DELAY_SECS=1 WAKEUP_IDLE_OVERRIDE=60 WAKEUP_ENABLED=0
sleep 2
if [ ! -s "$LOG" ]; then
  pass "enabled=false keeps the alarm silent"
else
  fail "enabled=false keeps the alarm silent" "$(cat "$LOG")"
fi
settle
